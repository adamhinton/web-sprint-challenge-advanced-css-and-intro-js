...
function getArtistByIndex(arr, indexNumber) {

      return `the artist at index ${indexNumber} is ${arr[indexNumber].name}`
}
...
function get20s(task4Array) {
  const newArray = [];
    for (let i= 0; i<task4Array.length; i++){
    if (task4Array[i]['years'].split(" - ")[0] >=1900 && task4Array[i]['years'].split(" - ")[0] <=2000 && task4Array[i]['years'].split(" - ")[1] >=1900 && task4Array[i]['years'].split(" - ")[1] <=2000){
      newArray.push(task4Array[i]['name'])
    }
}
return newArray
};
...
//don't know how to do this, try later

// console.log(get20s(artists));

//need to split years string in to two numbers 
    //task4Array.split(" - ")
...
function removeArtist(taskFiveArray, taskFiveIndexNumber) {
  taskFiveArray.splice(taskFiveArray, 1)
  return taskFiveArray.length
};
